...
lon <- ncvar_get(ncin, "lon", start=c(562), count=c(199))
nlon <- dim(lon)
head(lon)
...
lat <- ncvar_get(ncin, "lat", verbose = F, start=c(82), count=c(139))
nlat <- dim(lat)
head(lat)
print(c(nlon, nlat))
...
daily_rain.array <- ncvar_get( ncin, dname, start=c(562, 82,1), count=c(199,139,366))
